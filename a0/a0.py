...
def get_users(twitter, screen_names):
    ...
    for name in screen_names:
        response=robust_request(twitter,"users/show",{'screen_name':name},5)
        for r in response:
            send.append(r)    
    return send 
...
def get_friends(twitter, screen_name):
    ...
    response=robust_request(twitter,"friends/ids",{'screen_name':screen_name,'count':'5000'},5)
    for r in response:
        send.append(r)
        #print(sorted(send, key=int))
    return sorted(send, key=int)
...
def friend_overlap(users):
    ...
    for i in range (0, len(users)):
        for j in range(i+1,len(users)):
            ...
            #print(u['screen_name'])
            #s1.append(u['screen_name'])
            #c.update(u['friends'])
            u2=users[j]
            #print(u2['screen_name'])
            #s2.append(u2['screen_name'])
            c.update(u2['friends'])
            for x in list(c):
                if c[x]==1:
                    del c[x]
            ...
            #print(n)        
            #send.append([
            t=(u['screen_name'],u2['screen_name'],n)
            send.append(t)    
    return sorted(send,key=lambda x:(x[2]),reverse=True)        
    #print(send)
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    for x in u1['friends']:
        if x in u2['friends']:
             response=robust_request(twitter,"users/lookup",{'user_id':x},5)
             for r in response:
                return r['name']
    pass
...
def create_graph(users, friend_counts):
    ...
    G=nx.Graph()
    for u in users: 
        G.add_node(u['screen_name'],label=True)
...
def draw_network(graph, users, filename):
    ...
    pos=None
    for u in users:
        l[u['screen_name']]=u['screen_name']
        nx.draw(graph)
    nx.draw_networkx_labels(graph,nx.spring_layout(graph),l)
