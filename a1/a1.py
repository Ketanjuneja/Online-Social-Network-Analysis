...
def bfs(graph, root, max_depth):
    ...
    while level<=max_depth:
    	...
    	for x in graph.neighbors(parent):
    		if x in q:
    			q1.append(x)
    			node2distances[x]=level
    			node2num_paths[x]=1
    			q.remove(x)
    			
    			count=count+1
    			node2parents[x].append(parent)
    			    				
    		else:	
    			if node2distances[x]==level :
    				node2num_paths[x]=node2num_paths[x]+1
    				node2parents[x].append(parent)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    score=dict()
    l=deque()
    ###for x,y in sorted(node2distances.items(),key=lambda (k,v): (v,k),reverse=True)
    	###l.append(x)
    ###sorted_x = sorted(node2distances.items, )
    s1=sorted(node2distances,key=lambda x:node2distances[x],reverse=True)
    flag=True
    for i in s1:
    	score.setdefault(i, 1)
    max_depth=0
    for key in s1:
    	value=node2distances[key]
    	v=score[key]
    	if flag==True:
    		max_depth=value
    		flag=False

    	if node2num_paths[key] > 1:
    		v=score[key]/node2num_paths[key]

    	for j in  node2parents[key]:
    		score[j]=score[j]+v
    		
    		if j > key :
    			t=(key,j)
    			result[t]=score[key]/node2num_paths[key]
    		else:
    			t=(j,key)
    			result[t]=score[key]/node2num_paths[key]	
...
def approximate_betweenness(graph, max_depth):
    ...
    for x in graph.nodes():
    	...
    	for key,v1 in result.items():
    		
    		
    		if key in final:
    			final[key]=final[key]+v1
    		else:
    			final[key]=v1
    
    for key,v1 in final.items():	
    	final[key]=v1/2
...
def partition_girvan_newman(graph, max_depth):
    ...
    ##print(H.nodes())
    l=deque()
    r1=approximate_betweenness(H,max_depth)
    final=sorted(r1,key=lambda x:(r1[x],x),reverse=True)
    
    for i in final:
    	l.append(i)
    components = [c for c in nx.connected_component_subgraphs(H)]
    while len(components) == 1:
        edge_to_remove = l.popleft()
        ##print(indent + 'removing ' + str(edge_to_remove))
        H.remove_edge(*edge_to_remove)
        components = [c for c in nx.connected_component_subgraphs(H)]
    
    r = [c.nodes() for c in components]
    ##print(r)
    return components
    pass
...
def norm_cut(S, T, graph):
    ...
    ###TODO
    a= cut(S,T,graph)
    return float(a/volume(S,graph))+float(a/volume(T,graph))
    pass
...
def score_max_depths(graph, max_depths):
    ...
    for i in max_depths:
    	...
    	a=norm_cut(comp[0].nodes(),comp[1].nodes(),graph)
    	t=(i,a)
    	l.append(t)
    return l	
    pass
...
def make_training_graph(graph, test_node, n):
    ...
    ###TODO
    H=graph.copy()
    nodes= sorted(H.neighbors(test_node))
    q=deque()
    i=0
    flag=False
    for x in nodes:
...
def jaccard(graph, node, k):
    ...
    jaccard_scores = dict()
    final=[]
    ni = set(graph.neighbors(node))
    
    for x in graph.nodes():
    	
    	if x != node and not graph.has_edge(node, x):
    		nj = set(graph.neighbors(x))
    		
    		score = 1. * (len(ni & nj)) / (len(ni | nj))
    		
    		jaccard_scores[x]=score


    def key(elem):
    	return -elem[1],elem[0] 
    jack=sorted(jaccard_scores.items(),key=key)
    i=0
    for x,y in jack:
...
def path_score(graph, root, k, beta):
    ...
    scores = dict()
    for x in graph.nodes():
    	if x!=root and not graph.has_edge(root,x): 
    ...
    def key(elem):
    	return -elem[1],elem[0] 
    jack=sorted(scores.items(),key=key)		
    i=0
    for x,y in jack:
    	l.append(((root,x),y))
    	i=i+1
